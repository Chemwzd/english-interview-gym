...
> macOS 解压提示：推荐用系统自带方式解压（访达里直接双击 zip）。个别第三方解压工具（如 The Unarchiver）会漏解压内部目录；v0.15.9+ 安装包已做兼容处理，若遇到 `Failed to load Python shared library` 报错，用系统方式重新解压即可。
...
- 3 种人格（`materials/personas/`）：`hr-friendly` 外企 HR 初面（行为面）、`tech-lead` 技术主管面（研究深挖）、`stress` 压力面（终面）；每个人格都写明了定位、评估维度与追问策略，并内置一段真实面试对话 few-shot——也可以照格式换成你自己的目标岗位；
